...
This feature allows for the very useful assessment of nuanced market views, (e.g., that a stock's returns variance will increase over the next year, or that two stocks' returns correlation will decrease). Stress testing is also a potential application of this (e.g., if our investment strategy heavily depends upon assets A and B performing in an uncorrelated manner, so how could our portfolio be expected to fare in the event that A and B's correlation spiked to 0.5 over the next month?).
